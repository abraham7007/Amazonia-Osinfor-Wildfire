// Etapa 07 — Deep-sleep y memoria que sobrevive.
//
...
bool etapa_principal() {
  ...
  if (causa == ESP_SLEEP_WAKEUP_TIMER && g_marca == kMarca) {
    ...
    dato("etapa superada. Pulsa 'r' + ENTER para dormir otros 10 s.");
    return true;
  }

  dato("preparando: se escribe un patron en memoria RTC y se duerme 10 s");
  g_marca = kMarca;
  g_ciclos = 0;
...
void setup() { etapa("07", "Deep-sleep y memoria que sobrevive", etapa_principal); }
void loop() { reposo(); }
